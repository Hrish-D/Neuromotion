...
struct BaselineCalibrator {
    func computeBaseline(from frames: [FrameCapture]) -> [String: Double]? {
        ...
        return computeBaseline(fromEligibleFrames: validFrames)
    }

    func computeBaseline(fromEligibleFrames frames: [FrameCapture]) -> [String: Double]? {
        guard !frames.isEmpty else { return nil }
        let allKeys = Set(frames.flatMap { $0.rawBlendshapes.keys })
        var baseline: [String: Double] = [:]
        for key in allKeys {
            let values = frames.compactMap { $0.rawBlendshapes[key] }
